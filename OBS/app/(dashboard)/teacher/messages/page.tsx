...
import { createAnnouncement, getTeacherClasses, getTeacherAnnouncements } from "@/lib/actions/announcements";
...
export default function TeacherMessagesPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const result = await createAnnouncement(title, content, selectedClass);
        if (result.success) {
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="Tusaale: Imtixaan Berri ah..."
                                value={title}
                        ...
                        <div>
                            <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Fasalka (Target)</label>
                            <select
                                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl font-medium focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
                                value={selectedClass}
                                ...
                                <div key={msg.id} className="p-4 bg-slate-50 rounded-xl border border-slate-100 relative group hover:bg-white hover:shadow-md transition-all">
                                    ...
                                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-200/50">
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                                            Ku socota: {msg.classes?.name || "Dhammaan"}
                                        </span>
